...
function appendCompactToolbarActionOverlaysForRoot(
  ...
  // Rescue path: only synthesize overlays for toolbar buttons that were lost
  // or clipped out during serialization. Buttons that captured fine keep their
  // native snapshot (their text is realigned later by normalizeTextLineMetrics),
  // so a good button is never replaced by an inferior overlay or duplicated.
  ...
  for (const action of missingActions) {
    ...
    offsetSnapshotNode(overlay, offsetX, offsetY);
    anchorSnapshotToParent(overlay, rootSnapshot.rect);
    // Prune the clipped original when it exists; a genuinely missing button has
    // no node id, so nothing is pruned and only the overlay remains.
    overlays.push({ sourceId: getNodeId(action.element) ?? "", overlay });
  }
...
function createCompactToolbarActionOverlayFromRects(action: CompactToolbarAction): ElementSnapshot | null {
  ...
  const styles: Record<string, string> = {
    ...
    // Encode the measured text offset as paddingLeft + line-height so the Figma
    // importer (which places text from the box + text styles, not the text rect)
    // reproduces the button's layout instead of floating the label up-left.
